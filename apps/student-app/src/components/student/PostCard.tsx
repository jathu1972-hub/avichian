  ...
  const viewerActions = [
    {
      ...
      danger: true,
      onClick: () => {
        const reason = window.prompt('Report reason (SPAM, HARASSMENT, INAPPROPRIATE, OTHER)', 'INAPPROPRIATE');
        if (!reason) return;
        void reportPost(post.id, reason.toUpperCase())
  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-2 p-3 sm:gap-3 sm:p-4">
        ...
        {post.mediaUrl ? (
          /\.(mp4|webm|mov)(\?|$)/i.test(post.mediaUrl) || post.mediaUrl.includes('post-video') ? (
            <video
              src={resolveMediaUrl(post.mediaUrl) ?? post.mediaUrl}
              controls
              playsInline
              preload="metadata"
              className="media-contain w-full bg-black"
            />
          ) : (
            <img
              src={resolveMediaUrl(post.mediaUrl) ?? post.mediaUrl}
