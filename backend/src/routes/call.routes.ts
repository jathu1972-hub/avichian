...
callRouter.post('/:callId/livekit-token', async (req: AuthRequest, res, next) => {
  try {
    const data = await issueCallLiveKitToken(req.user!.id, routeParam(req.params.callId));
    res.json({ success: true, data });
  } catch (error) {
    next(error);
  }
