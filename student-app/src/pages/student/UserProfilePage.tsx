...
export function UserProfilePage() {
  ...
  async function handleAddFriend() {
    ...
    try {
      ...
      if (/already friends/i.test(msg)) {
        setProfile((p) => (p ? { ...p, isFriend: true, friendshipStatus: 'friends' } : p));
      }
      setError(msg);
    } finally {
  ...
  async function handleAcceptIncoming() {
    ...
    try {
      setActionLoading(true);
      setError('');
      const reqId = (profile as StudentProfile & { pendingRequestId?: string }).pendingRequestId;
      if (reqId) {
        await acceptFriendRequest(reqId);
      } else {
        await acceptFriendByUserId(profile.id);
      }
      setProfile((p) =>
        p
          ? {
